...
```bash
export REGISTRY=<REGISTRY_HOST>:5000/fraud-det-v4    # Private registry
export BUILD_VM=<SSH_USER>@<REGISTRY_HOST>                # Build VM SSH
...
kubectl run setup -n $NS --image=busybox --restart=Never --rm -it \
  --overrides='{"spec":{"volumes":[{"name":"fb","nfs":{"server":"<FLASHBLADE_NFS_IP>","path":"/financial-fraud-detection-demo"}}],"containers":[{"name":"setup","image":"busybox","command":["sh"],"volumeMounts":[{"name":"fb","mountPath":"/fb"}]}]}}' \
...
```bash
ssh -i $SSH_KEY $BUILD_VM "cd /home/<SSH_USER> && git clone https://github.com/SpearheadCorp/fraud-dev-v4.git"
```

### 3b. Build and Push All Images
```bash
ssh -i $SSH_KEY $BUILD_VM 'cd /home/<SSH_USER>/fraud-det-v4 && git pull && \
  REGISTRY=<REGISTRY_HOST>:5000/fraud-det-v4 && \
...
curl -s http://<REGISTRY_HOST>:5000/v2/_catalog | python3 -m json.tool

# List tags for a specific image
curl -s http://<REGISTRY_HOST>:5000/v2/fraud-det-v4/backend/tags/list
...
ssh -i $SSH_KEY <SSH_USER>@<K8S_WORKER_1> "nvidia-smi"
ssh -i $SSH_KEY <SSH_USER>@<K8S_WORKER_2> "nvidia-smi"

# Check GPU memory on a node
ssh -i $SSH_KEY <SSH_USER>@<K8S_WORKER_1> "nvidia-smi --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv"

# DCGM profiling metrics (1-second resolution GPU engine activity)
ssh -i $SSH_KEY <SSH_USER>@<K8S_WORKER_1> "dcgmi dmon -e 1001,1002,1003 -d 1000"
...
ip = os.environ.get('FLASHBLADE_MGMT_IP', '<FLASHBLADE_MGMT_IP>')
...
curl -s http://<REGISTRY_HOST>:5000/v2/_catalog
...
### NFS Volumes (FlashBlade @ <FLASHBLADE_NFS_IP>)

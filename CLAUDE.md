...
| ------ | --------------- | -------- | --------- | --------------------------- |
| `main` | `fraud-det-v31` | `30880`  | `latest`  | `http://<K8S_WORKER_1>:30880` |
| `dev`  | `fraud-det-dev` | `30881`  | `dev`     | `http://<K8S_WORKER_1>:30881` |

Build VM (all image builds happen here): `<SSH_USER>@<REGISTRY_HOST>`
...
The build VM (`<SSH_USER>@<REGISTRY_HOST>`) has two remotes:
...
> Build VM: `<REGISTRY_HOST>` | Namespace: `fraud-det-dev` | Dashboard: `http://<K8S_WORKER_1>:30881`
...
docker rmi <REGISTRY>/fraud-detection/<pod>:dev
...
```bash
cd /home/<SSH_USER>/fraud-det-v31
...
docker build --no-cache -t <REGISTRY>/fraud-detection/<pod>:dev -f pods/<pod>/Dockerfile . && \
docker push <REGISTRY>/fraud-detection/<pod>:dev
```
...
# Check metrics API
curl http://<K8S_WORKER_1>:30881/api/metrics | python3 -m json.tool
```

### Pipeline control (dev)

```powershell
curl -X POST http://<K8S_WORKER_1>:30881/api/control/start
curl -X POST http://<K8S_WORKER_1>:30881/api/control/stop
curl -X POST http://<K8S_WORKER_1>:30881/api/control/reset
```
...
The dashboard at `http://<K8S_WORKER_1>:30880` (prod) or `:30881` (dev) provides start/stop/reset controls via UI.
...
- **Prometheus:** `http://<K8S_WORKER_1>:30090` — DCGM scrape interval is 200ms

...
for multi-GPU parallelism. Processes transaction data generated by Pod 1.

This service demonstrates:
- Multi-GPU parallel processing with Dask-cuDF
- Pure Storage FlashBlade high-throughput reads
- Scalable feature engineering for ML pipelines
"""
...
class DataPrepService:
    def __init__(self, config: PrepConfig):
        ...
        # Initialize Dask for multi-GPU if available and enabled
    ...
    def _load_sampled_files(self, files: List[Path], file_type: str, max_files: int) -> cudf.DataFrame:
        ...
        # Use multi-GPU loading if available
    ...
    def _load_files_multi_gpu(self, files: List[Path]) -> Optional[cudf.DataFrame]:
        ...
        try:
            # Convert to string paths for dask_cudf
            file_paths = [str(f) for f in files]
            
            # Read with dask_cudf - automatically distributes across GPUs
            ddf = dask_cudf.read_parquet(
                file_paths,
                split_row_groups=True,  # Better parallelization
            )
            ...
            # Persist to GPU memory across cluster
            ddf = ddf.persist()
            wait(ddf)
            
            # Compute to single cuDF DataFrame
            # For feature engineering, we need a single DataFrame
